function ObTimeline(selector) {
	...
	var selection = d3.select(selector),
		...
		height = 300,
		timelineHeight = 25,
		parseDate = function(d){return d3.time.format("%m/%d/%Y").parse(d);},
		...
		valuesFilter = function(d){ //
			return valuesAccessor(d).filter(function(d){return yAccessor(d) != 0 && yAccessor(d) != null});
		},
	...
	function chart(data) {
		...
		line.defined(function(d) {return yAccessor(d) != null; })
		...
		timelineGroup = graphGroup.selectAll('.timeline-group')
			...
			.enter()
			.append('g')
			.attr("transform", function(d, i){return "translate(0," + (i * timelineHeight) + ")";})
			.attr('class', 'timeline-group').style('fill', function(d, i){return color(i)});
